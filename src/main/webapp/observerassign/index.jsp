...
<jsp:useBean id="support" class="uk.ac.leedsbeckett.ltitools.observerassign.ObserverAssignPageSupport" scope="request"/>
<% support.setRequest( request ); %>

<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>Observer Assignment</title>
...
const dynamicPageData = ${support.dynamicPageDataAsJSON};
    ...
    <script type="module" src="../javascript/@BUILDTIMESTAMP@/observerassign/index.js"></script>
  </head>
  <body>
    <div id="dialogdiv" class="dialogs">
      ...
      <div role="dialog" id="searchdialog" aria-labelledby="searchdialogLabel" aria-modal="true" class="hidden">
        ...
        <div>
          <table>
            <tr>
              <td valign="top">
                <h4>Which person do you want to observe your activity?</h4>
        ...
        <div class="dialog_form_actions">
          <button id="searchdialogObserveButton" value="Close">Observe</button>
      ...
      <div role="dialog" id="configdialog" aria-labelledby="configdialogLabel" aria-modal="true" class="hidden">
        <h3 id="configdialogLabel">Configure Observer Assignment Tool</h3>
        <div>
          <table>
    ...
    <h1>Observation Assignment Tool</h1>
    <div><p>This page is used by students to choose and assign observers to
        themselves. Typically an 'observer' is a specialist tutor from the
        student services department of Leeds Beckett University.</p></div>
    
    <c:choose>
      <c:when test="${support.allowedToAssignObserver}">
        
        <h2>People who are assigned as your observers</h2>
        <div><p>Not implemented.</p></div>

        <h2>People who can be assigned as observer</h2>
        <div><p>Not implemented.</p></div>
          
      </c:when>
      <c:otherwise>
        <p>Your role set by system that launched this tool means that you
        cannot assign observers.</p>
      </c:otherwise>
    </c:choose>
